...
LINE Harness は送信メッセージを `delivery_type` で区別します:

| delivery_type | 説明 | LINE 通数 |
|---------------|------|-----------|
| `push` | pushMessage（スケジュール配信、ブロードキャスト等） | **カウントされる** |
| `reply` | replyMessage（自動返信、友だち追加時の即時配信等） | **無料（カウントされない）** |

管理画面の「今月送信」は **push のみ** をカウントします。LINE 公式アカウントの通数と一致します。

### replyMessage が使われる場面

- 友だち追加時のウェルカムメッセージ（Step 0, delay=0）
- 自動返信（キーワードマッチ）
- ドミノフロー（ユーザーがボタンタップ → 次のメッセージ）

### 通数を節約するコツ

- ステップ配信の初回（delay=0）は replyMessage で送信される（無料）
- ユーザー起点のドミノフロー（ボタンタップチェーン）は全て replyMessage（無料）
- 時間指定のステップ配信（delay > 0）は pushMessage（有料）
